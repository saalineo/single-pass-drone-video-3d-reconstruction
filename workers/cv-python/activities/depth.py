...
async def infer_metric_depth(mission_id: str, set_id: str, attempt_id: str) -> dict:
    ...
    try:
        ...
        run_id = f"test-run-{mission_id}"

    scratch_dir = Path(settings.scratch_dir) / run_id / "depth"
    ...
    if await loop.run_in_executor(None, object_exists, manifest_key):
        try:
            existing_manifest = await loop.run_in_executor(None, get_json, manifest_key)
            manifest["frames"] = existing_manifest.get("frames", {})
        except Exception:
            pass
    ...
    for i, frame_id in enumerate(keyframe_ids):
        ...
        if str(frame_id) in manifest["frames"] and manifest["frames"][str(frame_id)].get("status") == "aligned":
            if await loop.run_in_executor(None, object_exists, key):
                continue
        
        try:
            rgb = await loop.run_in_executor(None, minio_io.get_keyframe, mission_id, set_id, frame_id)
        except Exception:
            manifest["frames"][str(frame_id)] = {"status": "unaligned", "reason": "failed to download keyframe"}
            continue
        ...
        try:
            ...
            manifest["frames"][str(frame_id)] = {"status": "unaligned", "reason": str(e)}
            continue
        ...
        manifest["frames"][str(frame_id)] = {"status": "aligned", **diag}

        # Periodic checkpoint of manifest every 10 frames or on completion
        if i % 10 == 0:
